...
5. Keep the global autonomy, escalation and scope policy in `gpts/kukutx/project-instructions.md`; domain Skills should add only domain-specific decision inputs, blockers and validation rules.
...
9. If a routing, ownership, autonomy or blocker boundary changes, update the smallest relevant `maintenance/behavioral-evals.md` case set.
10. When tuning for a newer model, remove duplicated generic process instructions before adding new ones. Domain Skills should encode durable decisions, not generic plan/ask/confirm loops.
11. Preserve existing project conventions and avoid unrelated rewrites.
12. Before completion run:
